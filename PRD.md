...
> **Example:** The SI lists 3 containers and 22,000 kg. The BL lists 4 containers and 22,000 kg. If the other fields agree, flag only the container count and show SI: 3 / BL: 4.
...
for email in inbox:    # reads each email record
    ...
inbox.read_text(path)  # returns the text from an SI or BL attachment
```
...
> **How to run it:** With the local server running, send your formatted output to `POST /submit` or call `inbox.submit(...)` through the loader. The response contains the evaluation result without exposing the reference answers.
...
- Review cases where the input was incomplete or uncertain. A score cannot fully assess whether the system asked for human review at the right time or provided enough context.
