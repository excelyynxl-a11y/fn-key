...
export default function ProcessingTimeline({ events = [] }) {
  ...
  return (
    <section className="mt-6 rounded-xl border border-sky-100 bg-sky-50/60 p-5">
      <h3 className="text-sm font-semibold text-slate-950">Processing timeline</h3>
      <ol className="mt-4 space-y-4 border-l border-sky-200 pl-4">
        {events.map((event) => (
          <li key={event._id ?? `${event.eventType}-${event.createdAt}`} className="relative">
            <span className="absolute -left-[21px] top-1 size-2.5 rounded-full bg-blue-500 ring-4 ring-sky-50" />
